...
# load data
terror=pd.read_csv('/Users/alibaba/gitlab/2018-mathmodels/q2/Q2_1.csv',encoding='ISO-8859-1')
print(terror[:4])
...
cleaned_dataset = cleaned_dataset[cleaned_dataset[:,0].argsort()]
print(type(cleaned_dataset))
row = len(cleaned_dataset)
cl = len(cleaned_dataset[0])
for idx, i in enumerate(cleaned_dataset):
    cleaned_dataset[idx][0] = idx
...
def gen_train(Groups):
    ...
    X_train1 = [cleaned_dataset[int(pair[0])] + cleaned_dataset[int(pair[1])] for pairs in train_pairs for pair in pairs] 
    print('number of x_train1 is %s' % len(X_train1))
    # 正反调换来一遍
    X_train2 = [np.append(pair[1], pair[0],axis =1) for pair in train_pairs] 
    # 两个特征差分试一下
    X_train = X_train1 #+ X_train2
    y_train = [y for y in y_train0 for yy in y] #+ y_train0
    
    return X_train, y_train

X_train, y_train = gen_train(train_groups)    
...
X_test, y_test = gen_train(test) 
print('nnum of test is %d' % len(y_test))   
